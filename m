Return-Path: <linux-acpi-owner@vger.kernel.org>
X-Original-To: lists+linux-acpi@lfdr.de
Delivered-To: lists+linux-acpi@lfdr.de
Received: from vger.kernel.org (vger.kernel.org [209.132.180.67])
	by mail.lfdr.de (Postfix) with ESMTP id 91BFFE33DB
	for <lists+linux-acpi@lfdr.de>; Thu, 24 Oct 2019 15:22:25 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S2502514AbfJXNWZ (ORCPT <rfc822;lists+linux-acpi@lfdr.de>);
        Thu, 24 Oct 2019 09:22:25 -0400
Received: from mx3.molgen.mpg.de ([141.14.17.11]:51745 "EHLO mx1.molgen.mpg.de"
        rhost-flags-OK-OK-OK-FAIL) by vger.kernel.org with ESMTP
        id S1730061AbfJXNWY (ORCPT <rfc822;linux-acpi@vger.kernel.org>);
        Thu, 24 Oct 2019 09:22:24 -0400
Received: from rabammel.molgen.mpg.de (rabammel.molgen.mpg.de [141.14.30.220])
        (using TLSv1.2 with cipher ECDHE-RSA-AES128-GCM-SHA256 (128/128 bits))
        (No client certificate requested)
        (Authenticated sender: pmenzel)
        by mx.molgen.mpg.de (Postfix) with ESMTPSA id 3BAB02000C194
        for <linux-acpi@vger.kernel.org>; Thu, 24 Oct 2019 15:22:21 +0200 (CEST)
To:     linux-acpi@vger.kernel.org
From:   Paul Menzel <pmenzel@molgen.mpg.de>
Subject: ACPI: [Firmware Bug]: BIOS _OSI(Linux) query ignored
Message-ID: <b253b43f-86e2-bab0-f30a-146dec1c0d71@molgen.mpg.de>
Date:   Thu, 24 Oct 2019 15:22:20 +0200
User-Agent: Mozilla/5.0 (X11; Linux x86_64; rv:68.0) Gecko/20100101
 Thunderbird/68.0
MIME-Version: 1.0
Content-Type: multipart/signed; protocol="application/pkcs7-signature"; micalg=sha-256; boundary="------------ms020601060802060600090803"
Sender: linux-acpi-owner@vger.kernel.org
Precedence: bulk
List-ID: <linux-acpi.vger.kernel.org>
X-Mailing-List: linux-acpi@vger.kernel.org

This is a cryptographically signed message in MIME format.

--------------ms020601060802060600090803
Content-Type: text/plain; charset=utf-8
Content-Language: en-US
Content-Transfer-Encoding: quoted-printable

Dear Linux folks,


What do the firmware creators have to provide, so that Linux
does not show the error message below anymore?

    ACPI: [Firmware Bug]: BIOS _OSI(Linux) query ignored

I believe, I have seen that on all my systems with proprietary
firmware, for example, Dell OptiPlex 5040 and Dell XPS 13 9380.

Should the warning be removed, as the vendors won=E2=80=99t implement
it, or are you in contact with the vendors to get them to fix
the firmware?


Kind regards,

Paul


--------------ms020601060802060600090803
Content-Type: application/pkcs7-signature; name="smime.p7s"
Content-Transfer-Encoding: base64
Content-Disposition: attachment; filename="smime.p7s"
Content-Description: S/MIME Cryptographic Signature


--------------ms020601060802060600090803--
